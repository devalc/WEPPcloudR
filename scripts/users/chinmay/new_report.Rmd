...
A large fraction of water was lost to `r totalwatsed_wbal$variable[2]` amounting  to `r round(totalwatsed_wbal$value[2])`% followed by `r totalwatsed_wbal$variable[3]` amounting to `r round(totalwatsed_wbal$value[3])`%.  
...
fig = plot_ly() %>% 
  ...
  layout(yaxis2=PrecipAx_d,legend = list(orientation = "h", x=0.05, y = 1.15,
                                         font = list(family = "Courier", size = 16, color = "black")), yaxis = list(title = ""))
fig
...
The table below shows runoff, sediment and phosphorus means for different land use categories.
...
The table below shows runoff, sediment and phosphorus means for different soil types.
...
The table below shows runoff, sediment and phosphorus means for different combinations of land use categories and soil types.
