...
This watershed received an average annual precipitation of `r round(totalwatsed_wbal$value[1])` mm over the `r (as.numeric(yr_max) - as.numeric(yr_min))+1` year simulation period between `r as.numeric(yr_min)` and `r as.numeric(yr_max)`. 
...
A large fraction of water was lost to `r tolower(gsub("[(%)]","",totalwatsed_wbal$variable[2]))` amounting  to `r round(totalwatsed_wbal$value[2])`% followed by `r tolower(gsub("[(%)]","",totalwatsed_wbal$variable[3]))` amounting to `r round(totalwatsed_wbal$value[3])`%.  
...
filter_slider("cumPercArea", "Filter map and table by contributing area (%)", sd_r, column=~cumPercArea, step=1, width=600)

ggplotly(ggplot() +
    geom_sf(data = sd_r, mapping = aes(fill = runoff_mm), show.legend = TRUE)  +
    coord_sf()+ggthemes::theme_map()+scale_fill_viridis_c(option = "A",alpha = 0.9))
...
# # Create a filter input
filter_slider("cumPercArea", "Filter map and table by contributing area (%)", sd, column=~cumPercArea, step=1, width=600)
...
```{r, echo=FALSE, message=FALSE,warning=FALSE}
# # Create a filter input
filter_slider("cumPercArea", "Filter map and table by contributing area (%)", sd_tp, column=~cumPercArea, step=1, width=600)
...
PrecipAx_d <- list(
       ...
       showgrid=FALSE, automargin = TRUE, color = "#8C564B"
   )
fig = plot_ly(colors = "viridis") %>% 
...
both_summary_tab = subcatchments   %>% as.data.frame() %>%
  dplyr::select(dplyr::any_of(c("landuse", "soil","slope",
                              "runoff_mm", "so_ls_kg_ha", 
                              "sd_yd_kg_ha", "tp_kg_ha")))%>%
    plyr::rename( c("landuse" ="Landuse",
                    "soil" ="Soil",
                   ...
                   "tp_kg_ha" ="Total Phosphorus (kg/ha)",
                  "runoff_mm"="Runoff (mm)"),warn_missing = FALSE)%>%
    group_by(Landuse, Soil) %>% dplyr::summarise_if(is.numeric, list(mean =
                                                                       mean)) %>%
                        dplyr::mutate_if(is.numeric, round, 2)
