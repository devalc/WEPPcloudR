...
landuse_summary_tab = subcatchments   %>% as.data.frame() %>%
    dplyr::select(landuse,slope,runoff_mm, so_ls_kg_ha, sd_yd_kg_ha, tp_kg_ha)%>%
...
soils_summary_tab = subcatchments   %>% as.data.frame() %>%
    dplyr::select(soil,slope,runoff_mm, so_ls_kg_ha, sd_yd_kg_ha, tp_kg_ha)%>%
...
both_summary_tab = subcatchments   %>% as.data.frame() %>%
    dplyr::select(landuse,soil,slope,runoff_mm, so_ls_kg_ha, sd_yd_kg_ha, tp_kg_ha)%>%
  dplyr::rename("Landuse" = "landuse",
                "Soil"="soil",
                "Slope" = "slope",
                "Soil Loss (kg/ha)"=  "so_ls_kg_ha",
