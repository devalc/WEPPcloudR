...
During the simulation period between `r as.numeric(yr_min)` and `r as.numeric(yr_max)`, `r as.numeric(stringr::str_extract_all(cli_sum, "\\d+([.,]\\d+)?")[[1]][1])` storms produced `r round(as.numeric(stringr::str_extract_all(cli_sum, "\\d+([.,]\\d+)?")[[1]][2]))` mm of rainfall on an average annual basis. `r as.numeric(stringr::str_extract_all(cli_sum, "\\d+([.,]\\d+)?")[[3]][1])` runoff events produced `r round(as.numeric(stringr::str_extract_all(cli_sum, "\\d+([.,]\\d+)?")[[3]][2]))` mm of runoff passing through the watershed outlet on an average annual basis.
...
ggplotly(ggplot() +
    ...
    coord_sf()+labs(fill = "Runoff (mm)") + ggthemes::theme_map()+scale_fill_viridis_c(option = "A",alpha = 0.9))
...
#### Precipitaiton, Streamflow,  Sediment Yield timeseries
...
```{r, echo=FALSE, message=FALSE,warning=FALSE} 
if(!file.exists(paste0("/geodata/weppcloud_runs/",params$proj_runid,"/wepp/runs/phosphorus.txt"))){
