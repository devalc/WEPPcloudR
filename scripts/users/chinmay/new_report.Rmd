...
"<span class='bigger'>" round(tail(df_head_percent(as.data.frame(tp_cp_df), 30),1)[,6]) " % </span> <span class='big_txt'>"  stringr::str_split(colnames(tp_cp_df)[6],"_")[[1]][2]  stringr::str_split(colnames(tp_cp_df)[6],"_")[[1]][3]
" is contributed by" round(tail(df_head_percent(as.data.frame(tp_cp_df), 30),1)[,5]) "% watershed area. </span>"


if(sum(tp_cp_df$Total_Phosphorus_kg)==0) {"-->"}

if(sum(tp_cp_df$Total_Phosphorus_kg)==0) {"<span class='big_txt'>No phosphorus was generated during the entire simulation.</span>"}
